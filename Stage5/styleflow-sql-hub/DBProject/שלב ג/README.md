...
## 2.ב. אימות והרצת שאילתות משלבים קודמים (Regression Verification)

על מנת לוודא כי תהליך האינטגרציה ומיזוג בסיסי הנתונים לא פגע בתקינות הנתונים הקיימים או בלוגיקת השאילתות של שלב ב' (Marketing Hub), הרצנו מחדש את השאילתות המשמעותיות והמורכבות מהשלב הקודם על מסד הנתונים המאוחד.

התוצאות והרכיבים שמרו על פעילות חלקה וקוהרנטית, מה שמוכיח את השמירה על שלמות הישויות והקשרים (Integrity Constraints) גם לאחר הוספת הטבלאות, הווריאציות, הגדרת רצפי ה-ID והשלמת ערכי המלאי המשולבים.

### 📸 צילומי מסך והוכחות ביצוע לשאילתות הבקרה:

* **הרצת שאילתות הבקרה המורכבות (חלק 1) ב-PostgreSQL:**
  ![הרצת שאילתות שלב ב מאומתות](../Stage3/screenShot/last_stage_queries.png)

* **הרצת שאילתות הבקרה ומול קמפיינים ולקוחות (חלק 2):**
  ![הרצת שאילתות שלב ב מאומתות חלק 2](../Stage3/screenShot/last_stage_queries2.png)

* **בקרת ריצה לניהול קמפיינים ותקציבים שיווקיים (חלק 3):**
  ![הרצת שאילתות שלב ב מאומתות חלק 3](../Stage3/screenShot/last_stage_queries3.png)

* **בחינת שאילתת הדגל המשולבת המשלבת את טבלת ה-Products הממוזגת:**
  ![שאילתת דגל אינטגרטיבית](../Stage3/screenShot/last_stage_queries_with%20_integration.png)
