...
-- 3. Remove inactive campaign links for branches that are no longer participating.
DELETE FROM campaign_branches
WHERE campaign_id IN (
    SELECT campaign_id FROM advertising_campaigns 
    WHERE status_id IN (SELECT status_id FROM campaign_status WHERE status_name = 'Cancelled')
);
